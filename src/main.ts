...
const products:Product[] = APIService.catalog.map((product:isProduct) => new Product (product.id, product.title, product.description, product.category, product.price, product.discountPercentage))



//Simulate clicking a product in the system to check.
function getProductInformation(product:Product){
...
function getAllProductInformation(){
    try{
        return products.map((product:Product) => getProductInformation(product))
